  ...
  return (
    <header className="relative bg-card/80 backdrop-blur-lg border-b sticky top-0 z-50 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-40">
        <div className="absolute inset-0 bg-gradient-to-r from-primary/20 via-transparent to-primary/10 animate-pulse"></div>
        <div className="absolute inset-0 bg-gradient-to-l from-accent/10 via-transparent to-accent/20 animate-[pulse_3s_ease-in-out_infinite]"></div>
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-primary/15 to-transparent animate-[shimmer_6s_ease-in-out_infinite]"></div>
      </div>
      ...
      {/* Mobile Menu */}
      <div id="mobile-menu" className={cn("relative md:hidden", {"block": isMenuOpen, "hidden": !isMenuOpen})}>
        <nav className="px-2 pt-2 pb-4 space-y-1 sm:px-3 border-t bg-card" aria-label="Mobile navigation">
          {navItems.map((item) => (
